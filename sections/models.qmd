...
As hypothesized, the logistic regression model with the expanded feature set performs worse than the baseline model. This indicates that the additional features might have introduced noise or overfitting. The model's performance is still quite good, but it is slightly worse than the baseline model.
...
We will now use the trained models to generate a list of the top N customers who are most likely to get a card (according to the model). We will compare the lists generated by each model to see if there is any overlap in the predictions.
...
reduced_best_model_trainer = (
    Trainer(
        all_cols_data_module,
        RandomForestClassifier(**gs_trainer.get_best_params()),
...
```{python}
# reduced_best_model_visualizer.plot_grid_search(log_scale_params=["n_estimators", "max_depth", "min_samples_split", "min_samples_leaf"])
```
...
```{python}
candidate_trainers.append(reduced_best_model_trainer)
