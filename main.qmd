...
def plot_categorical_variables(df, categorical_columns, fill_na_value="NA"):
    ...
    fig, axes = plt.subplots(
        nrows=len(categorical_columns), figsize=(14, len(categorical_columns) * 4.5)
    )

    if (
        len(categorical_columns) == 1
    ):  # If there's only one categorical variable, wrap axes in a list
        axes = [axes]
...
def match_cardholders_with_non_cardholders(non_transactional, transactions, top_n=5):
    ...
    for idx, cardholder in tqdm(
        ...
        cardholder_vector = (
            cardholder[eligible_cols].fillna(0).astype(bool).values.reshape(1, -1)
        )
        non_cardholder__matrix = (
            without_card_activity[eligible_cols].fillna(0).astype(bool).values
        )
        assert (
            cardholder_vector.shape[1] == non_cardholder__matrix.shape[1]
        ), "Dimension mismatch between cardholder and applicant activity matrix."

        distances = pairwise_distances(
            cardholder_vector, non_cardholder__matrix, metric="jaccard"
        ).flatten()
        eligible_non_cardholders = [
            i
...
```{python}
# remove all column that have card in them except for has_card
golden_record_df = golden_record_df.loc[:, ~golden_record_df.columns.str.contains("card") | golden_record_df.columns.str.contains("has_card")]
golden_record_df.isna().sum()
```

## Remove Time-Dependent Features
...
print(f"Removed {num_cols_before - len(golden_record_df.columns)} time-dependent columns. Now {len(golden_record_df.columns)} columns remain.")
```

## Remove STD Columns

```{python}
std_cols = [col for col in golden_record_df.columns if "std" in col]
num_cols_before = len(golden_record_df.columns)
golden_record_df = golden_record_df.drop(columns=std_cols)
print(f"Removed {num_cols_before - len(golden_record_df.columns)} std columns. Now {len(golden_record_df.columns)} columns remain.")
```
...
class Trainer:
    def __init__(self, data_module, model, cv=10, use_rfe=False, n_features_to_select=None, verbose=False):
    ...
    def _create_preprocessor(self):
        numerical_features = [col for col in self.data_module.X_train.columns if self.data_module.X_train[col].dtype in ["int64", "float64"]]
        categorical_features = [col for col in self.data_module.X_train.columns if col not in numerical_features]
        
        other_features = [col for col in self.data_module.X_train.columns if col not in numerical_features + categorical_features]
    ...
    def fit(self):
        ...
        if self.use_rfe:
            ...
            model_pipeline.steps.insert(0, ("rfe", rfecv))
    ...
    def eval(self):        
        X_train, X_test, y_train, y_test = (
            self.data_module.X_train, self.data_module.X_test, self.data_module.y_train, self.data_module.y_test)
        ...
        train_metrics_summary = cross_validate(
            self.pipeline, X_train, y_train, scoring=scoring, cv=self.cv, 
        ...
        self.metrics_report = {
            "train": {metric: {"folds": train_metrics_summary[f"test_{metric}"].tolist(), 
    ...
    def get_selected_features(self):
        if not self.use_rfe:
            return None
        rfe = self.pipeline.named_steps['model_pipeline'].named_steps['rfe']
        all_features = np.array(self.preprocessor.get_feature_names_out())
        selected_features = all_features[rfe.support_]
        
        # get original column names based on closest match with the selected features
        original_features = []
        for feature in selected_features:
            matches = [col for col in self.data_module.X_train.columns if col in feature]
            original_features.append(matches[0] if matches else feature)
            
        return np.array(list(set(original_features))), selected_features
...
class Visualizer:
    def __init__(self, trainer, model_name):
        ...
        X_train, X_test, y_train, y_test = (
            self.trainer.data_module.X_train, self.trainer.data_module.X_test, self.trainer.data_module.y_train, self.trainer.data_module.y_test)
        
        self.explainer = dx.Explainer(trainer.get_trained_model(), X_test, y_test)
        
        self.X_test = X_test
        self.y_true = y_test
        self.y_pred_proba = trainer.get_trained_model().predict_proba(X_test)

    @staticmethod
    def prepare_plot_layout(num_plots, num_cols=2):
    ...
    def visualize_performance(self, num_plots=4):
    ...
    def visualize_explanations(self, feature_columns = []):
        ...
        # Breakdown Plot
        observation = self.trainer.test_df[self.trainer.data_module.feature_columns].iloc[0]  
...
```{python}
## LEGACY SUPPORT
def train_evaluate_model(data_module, model, cv=10, use_rfe=False, n_features_to_select=None):
    trainer = (Trainer(data_module, model, use_rfe=use_rfe, n_features_to_select=n_features_to_select, cv=cv, verbose=True)
               .fit())
    trainer.eval()
    return trainer.get_metrics_report(), trainer

def visualize_results(trainer, model_name, viz_explanations=True):
...
baseline_data_module = create_data_module(golden_record_df, feature_columns)

baseline_metrics_report, baseline_trainer = train_evaluate_model(baseline_data_module, baseline_model)
visualize_results(baseline_trainer, "Baseline Logistic Regression")
```

```{python}
all_cols_data_module = create_data_module(golden_record_df, 
                                          golden_record_df.drop(columns=["has_card"]).columns)

baseline_trainer_rfe = (Trainer(all_cols_data_module, 
                               LogisticRegression(max_iter=10000), 
                               use_rfe=True, n_features_to_select=None, cv=2, verbose=False)
                        .fit())


baseline_trainer_rfe.eval()

visualizer = Visualizer(baseline_trainer_rfe, "Baseline Logistic Regression with RFE")
visualizer.visualize_performance()

print("Selected Features:", baseline_trainer_rfe.get_selected_features())
```
...
rand_forest_metrics_report, rand_forest_trainer = train_evaluate_model(
    data_module, model, cv=5, use_rfe=False, n_features_to_select=None
)
...
decision_tree_trainer = (Trainer(data_module, decision_tree_model, cv=5, use_rfe=False, n_features_to_select=None)
                         .fit()
                         .eval())
...
gradient_boost_trainer = (Trainer(data_module, gradient_boost_model, cv=5, use_rfe=False, n_features_to_select=None)
                          .fit()
                          .eval())

visualize_results(gradient_boost_trainer, "Gradient Boosting")
```
