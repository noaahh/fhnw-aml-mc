...
def create_train_test_sets(df, target_col="has_card", test_size=0.2):
    """
    Create train and test sets for machine learning model training.
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=test_size, random_state=1337, stratify=y, shuffle=True
    )

    return X_train, X_test, y_train, y_test

X_train, X_test, y_train, y_test = create_train_test_sets(golden_record_df)
...
def create_pipeline_steps(model, feature_columns, train_data_df, cv_k, include_rfecv=False):
    """
    Create a scikit-learn pipeline for preprocessing and model training.
    Optionally includes Recursive Feature Elimination with cross-validation.

    Parameters:
    - model: The machine learning model to be trained.
    - feature_columns: List of column names to be used as features.
    - train_data_df: DataFrame containing the training data.
    - cv_k: Number of cross-validation folds.
    - include_rfecv: Boolean to include RFECV in the pipeline.

    Returns:
    - List of tuples: Steps for the scikit-learn pipeline.
    """
    missing_features = set(feature_columns) - set(train_data_df.columns)
    if missing_features:
        raise ValueError(f"Features missing in DataFrame: {missing_features}")
    
    numerical_features = [
        col for col in feature_columns if train_data_df[col].dtype in ["int64", "float64"]
    ]
    categorical_features = [
        col for col in feature_columns if train_data_df[col].dtype == "object"
    ]
    ...
    if include_rfecv:
        rfecv = RFECV(estimator=model, step=1, cv=KFold(cv_k), scoring='accuracy', n_jobs=-1, verbose=1)
...
def train_evaluate_model(
    ...
    pipeline_steps = create_pipeline_steps(estimator, feature_columns, X_train, cv, include_rfecv)
    ...
    y_pred_proba = (
        pipeline.predict_proba(X_test) if hasattr(pipeline, "predict_proba") else np.nan
    )
...
def visualize_results(metrics_report, model_name, cv=5):
    """
    ...
    for i, (mean, std) in enumerate(zip(val_means, val_stds)):
        plt.text(
            i, mean + std + 0.01, f"{mean:.2f} ± {std:.2f}", ha="center", va="bottom"
        )
    plt.ylim(0, 1)
    plt.grid(True)
    plt.show()
...
def visualise_cm(y_true, y_pred_proba, title):
    skplt.metrics.plot_confusion_matrix(y_true, y_pred_proba, figsize=(10, 6))
    plt.title(title)
    plt.show()
    
def visualise_roc_curve(y_true, y_pred_proba, title):
    skplt.metrics.plot_roc(y_true, y_pred_proba, figsize=(10, 6))
    plt.title(title)
    plt.show()
    
def visualise_precision_recall_curve(y_true, y_pred_proba, title):
...
```{python}
feature_columns = (
    [  # Reducing the number of features for the baseline model as per task requirements
        "age",
        "sex",
        "client_region",
    ]
...
baseline_metrics_report = train_evaluate_model(
    baseline_model, X_train, y_train, X_test, y_test, feature_columns, include_rfecv=False
)
visualize_results(
    baseline_metrics_report, "Baseline Logistic Regression"
)
```
...
baseline_metrics_report = train_evaluate_model(
    baseline_model, X_train, y_train, X_test, y_test, feature_columns, include_rfecv=True
)

visualize_results(
    baseline_metrics_report, "Baseline Logistic Regression with RFECV"
)
```

```{python}
np.array(baseline_metrics_report["pipeline"].named_steps["preprocessor"].get_feature_names_out())
```

```{python}
baseline_metrics_report["pipeline"].named_steps["rfecv"].support_
```

```{python}
available_features = np.array(baseline_metrics_report["pipeline"].named_steps["preprocessor"].get_feature_names_out())
selected_features = available_features[baseline_metrics_report["pipeline"].named_steps["rfecv"].support_]
...
rf_metrics_report, y_test, y_pred_proba = train_evaluate_model(
    X_train, y_train, X_test, y_test, feature_columns, random_forest_model
)
```
...
gb_metrics_report, y_test, y_pred_proba = train_evaluate_model(
    X_train, y_train, X_test, y_test, feature_columns, gradient_boost_model
)
```
