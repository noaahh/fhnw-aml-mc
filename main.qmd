...
def create_pipeline_steps(model, feature_columns, train_data_df, cv_k, include_rfecv=False):
    ...
    if include_rfecv:
        rfecv = RFECV(estimator=model, step=1, cv=StratifiedKFold(cv_k), scoring='accuracy', n_jobs=-1, verbose=1)
...
def train_evaluate_model(
    ...
    pipeline_steps = create_pipeline_steps(estimator, feature_columns, train_df, cv, include_rfecv)
...
def visualize_results(metrics_report, model_name, cv=10):
    """
...
baseline_metrics_report = train_evaluate_model(
    baseline_model, train_df, test_df, feature_columns
)
visualize_results(
...
feature_columns = train_df.columns.drop("has_card").tolist()

baseline_metrics_report = train_evaluate_model(
    baseline_model, train_df, test_df, feature_columns, include_rfecv=True
)
...
```{python}
selected_features = baseline_metrics_report["pipeline"]["rfecv"].support_
print(f"Number of selected features: {selected_features.sum()}")

baseline_metrics_report["pipeline"]["rfecv"].ranking_
```
...
rf_metrics_report, y_test, y_pred_proba = train_evaluate_model(
    train_df, test_df, feature_columns, random_forest_model
)
```
...
gb_metrics_report, y_test, y_pred_proba = train_evaluate_model(
    train_df, test_df, feature_columns, gradient_boost_model
)
```
