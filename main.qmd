...
```{python}
def aggregate_transactions_monthly(df, validate=True):
    """
...
def visualize_results(metrics_report, model_name, y_true, y_pred_proba, cv=10):
    ...
    skplt.metrics.plot_confusion_matrix(y_true, preds, figsize=(10, 6))
    ...
    if not np.isnan(y_pred_proba).all():
        ...
        plt.title(f'{model_name}: ROC Curve on Test Set')
        plt.show()

        # Plot precision-recall curve
        skplt.metrics.plot_precision_recall(y_true, y_pred_proba, figsize=(10, 6))
        plt.title(f'{model_name}: Precision-Recall Curve on Test Set')
        plt.show()

        # Plot lift curve
        skplt.metrics.plot_lift_curve(y_true, y_pred_proba, figsize=(10, 6))
        plt.title(f'{model_name}: Lift Curve on Test Set')
        plt.show()

        # Plot cumulative gain curve
        skplt.metrics.plot_cumulative_gain(y_true, y_pred_proba, figsize=(10, 6))
        plt.title(f'{model_name}: Cumulative Gain Curve on Test Set')
        plt.show()
```
...
feature_columns = [ # Reducing the number of features for the baseline model as per task requirements
    'age',
    'client_region'
] + [col for col in golden_record_df.columns if 'M_' in col and ('_balance' in col or '_volume' in col)]

baseline_metrics_report, y_test, y_pred_proba = train_evaluate_model(train_df, test_df, feature_columns, baseline_model)
visualize_results(metrics_report, "Baseline: Logistic Regression", y_test, y_pred_proba)
```
